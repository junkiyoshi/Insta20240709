...
void ofApp::update() {
	...
	for (int i = this->location_list.size() - 1; i >= 0; i--) {
		...
		if (this->radius_list[i] > 0) {
			...
			this->radius_list[i] += this->speed_list[i] * 2;
		}
		else {
	...
	for (int i = 0; i < this->noise_seed_list.size(); i++) {
		...
		auto span = 360 / this->noise_seed_list.size();
		auto random_start = i * span;
		auto random_end = i * span + span;

		color.setHsb(ofRandom(random_start, random_end), 200, 255);
